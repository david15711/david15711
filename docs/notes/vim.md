...
| ;                      | 마지막으로 사용한 t/T, f/F [N] 번 시행                          |
| ,                      | 마지막으로 사용한 t/T, f/F (역방향)                             |
| t / T                  | 행 앞으로 가며 찾아 그 전까지 이동 / 반대 방향                  |
| M                      | 화면 가운데의 맨 앞으로                                         |
| z<CR>                  | 커서 위치가 화면 맨 위로 정렬                                   |
| zz                     | 커서 위치로 화면 정렬                                           |
...
| g{number}g / {number}G | {number}줄로 이동, 없으면 파일 맨 앞 / 뒤로 이동                |
| /                      | 검색 모드                                                       |
| ?                      | 검색 모드 (역순)                                                |
| *                      | 커서 위의 단어 찾기                                             |
| #                      | 커서 위의 단어 찾기 (역순)                                      |
| n / N                  | 검색한 단어로 이동 앞으로 / 뒤로 (역순 검색에서는 거꾸로)       |
| m{a-zA-Z}              | 현재 커서 위치에 해당 알파벳을 태그로 마크 설정 (해당 register) |
| `{a-zA-Z}              | 설정했던 마커로 이동                                            |
| `` / ''                | 줄 점프하기 전으로 이동                                         |
...
| key       | behavior                                                                                |
| --------- | --------------------------------------------------------------------------------------- |
| [[ / ]]   | 이전/다음 코드 섹션(또는 1열에 있는 { / })으로 이동                                     |
| [{ / }]   | 현재 있는 블록의 시작/끝 바깥쪽 중괄호 { } 위치로 점프                                  |
| [( / )]   | 현재 있는 괄호 ( ) 의 시작/끝 위치로 점프                                               |
| [] / ][   | 이전/다음 섹션의 끝(})으로 이동                                                         |
| [m / ]m   | 이전/다음 메서드(Method/Function) 시작 부분으로 이동 (자바, C++ 등)                     |
| [M / ]M   | 이전/다음 메서드 끝 부분으로 이동                                                       |
| [c / ]c   | 이전/다음 변경 사항(Diff change)으로 이동 (Vimdiff 사용 시)                             |
| [d / ]d   | 커서 아래 단어의 마크로/매크로 정의(Define) 위치로 이동                                 |
| [i / ]i   | 커서 아래 단어가 포함된 첫 번째 포함(Include) 선언 또는 변수 선언 위치 검색             |
| i[[](){}] | (visual 또는 d, c, y 명령어 상태에서) 선택한 괄호 안쪽 내용 선택 (Inner bracket)        |
| a[[](){}] | (visual 또는 d, c, y 명령어 상태에서) 선택한 괄호 내용 전체(괄호 포함) 선택 (A bracket) |
...
| dd                 | 문장 줄 지우기 / 1번 레지스터에 저장됨 ("1로 사용)                                         |
| c {motion} / C     | c 이후 {motion}에 따라 바꾸기 / 커서 뒤로 행 끝까지 바꾸기                                 |
| cc                 | 문장 줄 지우고 삽입                                                                        |
| s / S              | char 바꾸기/ 행 전체 바꾸기                                                                |
| u                  | undo                                                                                       |
| U                  | 줄단위 undo, 다시 하면 줄단위 redo, 특수 행동으로 취급되어 행동 스택에 쌓임                |
| CTRL-R             | redo                                                                                       |
| ~                  | 하나 대소문자 바꾸기                                                                       |
...
| &                  | 마지막으로 진행했던 :s (substitution) 반복                                                 |
| v / V / CTRL-V     | 비주얼 모드 / 비주얼 라인 모드 / 비주얼 블록 모드 (CTRL-V가 붙여넣기인 환경에서 CTRL-Q)    |
...
| :norm {commands}   | {commands} 일괄 적용, :1,10norm A; 시 1부터 10까지의 줄에서 명령어 A(Append)후 ; 작성 동작 |

#### 반복 매크로
| key               | behavior                                                        |
| ----------------- | --------------------------------------------------------------- |
| q{0-9A-Za-z"}     | q뒤에 쓴 식별드의 레지스터로 동작을 기록한다 q로 recording 종료 |
| @{0-9A-Za-z".=*+} | record한 레지스터의 동작을 다시 반복한다, [N]                   |
...
| key        | description                                                 |
| ---------- | ----------------------------------------------------------- |
| [N] CTRL-E | [N] 만큼 화면을 아래로 스크롤                               |
| CTRL-D     | buffer에 있는 페이지의 절반 이동, [N] 입력 시 CTRL-E와 같음 |
| CTRL-F     | buffer에 있는 페이지 하나 이동,                             |
| [N] CTRL-Y | [N] 만큼 화면을 위로 스크롤                                 |
| CTRL-U     | buffer에 있는 페이지의 절반 이동, [N] 입력 시 CTRL-Y와 같음 |
...
| {number} CTRL-W c        | {number}번째의 split 화면 닫기, 없으면 현재 화면 닫기                         |
| CTRL-W {number} w/W      | {number}번째의 화면으로 이동, 없으면 화면간 이동 (우하단/좌상단)              |
| CTRL-W t/b/h/j/k/l       | 분할된 화면 간 이동, 최상단/최하단/좌/하/상/우                                |
| CTRL-W CTRL-^            | 현재 파일 분할 및 대체 파일 편집 (마지막 버퍼에 있는 파일)                    |
...
| :tabm [{number}$#+-] | 현재 탭을 하나 이동 / {number} 위치로 이동 $:마지막 #:마지막본탭우측 +:우측 -:좌측 |
| :tabs                | 현재 탭 리스트 표시                                                                |
...
| prefix      | description                                              |
| ----------- | -------------------------------------------------------- |
| g:          | Global - 전역 변수 (모든 버퍼/창에서 접근 가능)          |
| b:          | Buffer - 현재 버퍼(파일)에만 유효한 변수                 |
| w:          | Window - 현재 창(Window)에만 유효한 변수                 |
| t:          | Tab - 현재 탭 페이지에만 유효한 변수                     |
| v:          | Vim - Vim이 미리 정의해 둔 전역 변수                     |
| l:          | Local - 함수(Function) 내 지역 변수                      |
| a:          | Argument - 함수 전달 인자 (읽기 전용)                    |
| &{variable} | 옵션 변수 접근, Vim의 set 옵션도 변수처럼 읽고 쓰기 가능 |
| @{variable} | 레지스터 접근                                            |
| ${variable} | 환경 변수 접근                                           |
...
| key               | description                                                                     |
| ----------------- | ------------------------------------------------------------------------------- |
| :!{commands}      | 외부 서브쉘을 열어서 {commands}를 실행한다. 파일 전체를 리다이렉션              |
